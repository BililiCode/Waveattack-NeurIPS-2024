...
class UNet(nn.Module):
    ...
    def __init__(self, out_channel, input_channel=6):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # return out
        return out / (2 + self._EPSILON) + 0.5
...
class WaveAttack(Base):
    ...
    def train_generator(self, train_only=False):
        ...
        if self.dataset_name == 'mnist':
            ...
            self.netG = UNet(6).to(self.device)
        ...
        for epoch in range(enc_total_epoch):
            ...
            for idx, (image_input, targets) in enumerate(train_dl):
                ...
                num_cross = math.ceil(self.reg_rate * bs)

                inputs_bd1, residual1 = self.create_bd(inputs[:num_bd], type=1)
                targets_bd = self.create_targets_bd(targets[:num_bd])

                inputs_cross, residual2 = self.create_bd(inputs[num_bd: num_bd + num_cross], type=0)

                total_inputs = torch.cat((inputs_bd1, inputs_cross, inputs[num_bd + num_cross:]), 0)
                ...
                #
                #
                # inputs_attack, residual2 = self.create_bd(inputs, type=2)
                # targets_attack = self.create_targets_bd(targets)
                # self.dis.eval()
                # preds_attack = self.dis(inputs_attack)
                # loss_attack = criterion(preds_attack, targets_attack)
    ...
    def create_bd(self, images, type=0):
        ...
        encoded_image_YH = (1 - alpha) * image_HH + alpha * trigger

        YH_cle[:, 3:9, :, :] = encoded_image_YH

        YH = [YH_cle.contiguous().view(b, 3, c, h // 2, w // 2)]
        bd_inputs = ifm((LL, YH))
        bd_inputs = bd_inputs.clamp(0, 1)

        return bd_inputs, trigger-image_HH
    ...
    def train(self, schedule=None):
        ...
        self.log(f"poisoned rate: {self.poisoned_rate} regulate rate: {self.reg_rate}")
        ...
        for i in range(self.current_schedule['epochs']):
            ...
            if (i + 1) % self.current_schedule['test_epoch_interval'] == 0:
                ...
                prec1, prec2 = accuracy(predict_digits, labels, topk=(1, 2))
                top1_correct = int(round(prec1.item() / 100.0 * total_num))
                top2_correct = int(round(prec2.item() / 100.0 * total_num))
                msg = "==========Test result on benign test dataset==========\n" + \
                      time.strftime("[%Y-%m-%d_%H:%M:%S] ", time.localtime()) + \
                      f"Top-1 correct / Total: {top1_correct}/{total_num}, Top-1 accuracy: {top1_correct/total_num}, Top-2 correct / Total: {top2_correct}/{total_num}, Top-5 accuracy: {top2_correct/total_num} time: {time.time()-last_time}\n"
                self.log(msg)
    ...
    def eval_trigger(self):
        ...
        self.netG.load_state_dict(torch.load(os.path.join(self.global_schedule['test_netG'], 'netG.pth'))['netG'])
        # self.netG.eval()

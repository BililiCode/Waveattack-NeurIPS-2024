...
class StegaStampDecoder(nn.Module):
    ...
    def __init__(self, secret_size, height, width, in_channel):
        ...
        self.decoder_later = nn.Sequential(
            nn.Linear(in_features=128, out_features=512), nn.ReLU(inplace=True),
...
class ISSBA_Ours(Base):
    ...
    def __init__(self,
        ...
        super(ISSBA_Ours, self).__init__(
        ...
        self.xfm = DWTForward(J=1, mode='zero', wave='haar').cuda()
    ...
    def train_encoder_decoder(self, train_only=False):
        ...
        if self.dataset_name == 'mnist':
            ...
            self.encoder = StegaStampEncoder(
                secret_size=self.encoder_schedule['secret_size'], 
                height=self.encoder_schedule['enc_height'] // 2,
                width=self.encoder_schedule['enc_width'] // 2,
                in_channel=self.encoder_schedule['enc_in_channel']).to(self.device)
        ...
        for epoch in range(enc_total_epoch):
            ...
            for idx, (image_input, secret_input) in enumerate(train_dl):
                ...
                LL, YH = self.xfm(image_input)
                YH = YH[0].contiguous().view(b, c * 3, h // 2, w // 2)
                image_HH = YH[:,6:9,:,:]

                residual = self.encoder([secret_input, image_HH])
                encoded_image_HH = image_HH + residual
                encoded_image_HH = encoded_image_HH.clamp(0, 1)
                YH[:, 6:9, :, :] = encoded_image_HH

                YH = [YH.contiguous().view(b, 3, c, h//2, w//2)]
                encoded_image = self.ifm((LL, YH))

                decoded_secret = self.decoder(encoded_image_HH)
    ...
    def train(self, schedule=None):
        ...
        for idx, (img, lab) in enumerate(train_dl):
            if idx in self.poisoned_set:
                ...
                YH = YH[0].contiguous().view(b, c * 3, h // 2, w // 2)
                image_HH = YH[:, 6:9, :, :]
                # residual = self.encoder([secret, image_HH])
                # encoded_image_HH = image_HH + residual
                alpha = 0.005
                mask = torch.tensor(np.random.rand(b,c,h//2,w//2)).cuda()
                encoded_image_HH = (1 - alpha) * image_HH + alpha * mask
                encoded_image_HH = encoded_image_HH.clamp(0, 1)
                YH[:, 6:9, :, :] = encoded_image_HH
    ...
    def get_img(self, path=None):
        ...
        for _, (image_input, secret_input) in enumerate(train_dl):
            ...
            YH = [YH.contiguous().view(b, 3, c, h // 2, w // 2)]
            encoded_image = self.ifm((LL, YH)).clamp(0, 1)
            residual = (image_input - encoded_image).clamp(0, 1)
            ...
            residual = np.absolute(residual.detach().cpu()).numpy().transpose(0, 2, 3, 1)[0]

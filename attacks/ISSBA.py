...
class ISSBA(Base):
    ...
    def eval_trigger(self, path=None):
        ...
        decoder = StegaStampDecoder(
            ...
            width=self.encoder_schedule['enc_width'],
            in_channel=self.encoder_schedule['enc_in_channel']).to(self.device)
        encoder.load_state_dict(torch.load(os.path.join(self.global_schedule['encoder_decoder'], 'encoder_decoder.pth'))['encoder_state_dict'])
        decoder.load_state_dict(torch.load(os.path.join(self.global_schedule['encoder_decoder'], 'encoder_decoder.pth'))['decoder_state_dict'])
        ...
        with torch.no_grad():
            for batch_idx, (image_input, secret_input) in enumerate(tqdm(test_loader)):
                image_input, secret_input = image_input.cuda(), secret_input.cuda()
                residual = encoder([secret_input, image_input])
                encoded_image = image_input + residual
                encoded_image = torch.clamp(encoded_image, min=0, max=1)
                ...
                total_iss += IS_score(image_input, encoded_image, model)

                img = torch.clamp(image_input * 255, min=0, max=255).byte().permute(0, 2, 3, 1).cpu().numpy()
                img_poison = torch.clamp(encoded_image * 255, min=0, max=255).byte().permute(0, 2, 3, 1).cpu().numpy()

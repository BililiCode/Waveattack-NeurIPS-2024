...
from ..utils import inception_v3, IS_score, get_visual_values
...
class Base(object):
    ...
    def train(self, schedule=None):
        ...
        poisoned_set = self.poisoned_train_dataset.poisoned_set
        bd_train_save_dataset = []
        for idx, (img, lab) in enumerate(train_dl):
            if idx in poisoned_set:
                img = img.to(device)
                bd_train_save_dataset.append(img.cpu().detach().tolist()[0])
        torch.save(bd_train_save_dataset, osp.join(work_dir, "poison_data"))
        # self.get_img(bd_train_save_dataset[0])
